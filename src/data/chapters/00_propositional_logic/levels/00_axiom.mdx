...
export const axiom = parseFormula('p->q');
...
<Speak speaker="sapphire">
<LocalizedMarkdown locale="en">
Exactly. Also, the $\vdash$ symbol is read as **proves**. You can also say that the subsequent string can be **proved** from the preceding ones. Actually afterwards we might as well speak of **proving** a string instead of **producing** it. 
</LocalizedMarkdown>
<LocalizedMarkdown locale="zh">
对的。以及，这个 $\vdash$ 符号读作**证明**。你也可以说后面的字符串可以被前面的**证明**出来。事实上我们之后要不就说**证明**一个字符串，而不是**制造**它。
</LocalizedMarkdown>
</Speak>
